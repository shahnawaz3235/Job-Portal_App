...
const jobSlice = createSlice({
  ...
  reducers: {
    ...
    successForPostJob(state, action) {
      state.message = action.payload.jobs;
      state.error = null;
      state.loading = false;
    },
...
export const fetchJobs =
  ...
  async (dispatch) => {
    try {
      ...
      let link = `${import.meta.env.VITE_REACT_APP_BASE_URL}/api/v1/job/getall?`;
      let queryParams = [];
      if (searchKeyword) {
...
export const fetchSingleJob = (jobId) => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `${import.meta.env.VITE_REACT_APP_BASE_URL}/api/v1/job/get/${jobId}`,
...
export const postJob = (data) => async (dispatch) => {
  ...
  try {
    const response = await axios.post(
      `${import.meta.env.VITE_REACT_APP_BASE_URL}/api/v1/job/post`,
      data,
...
export const getMyJobs = () => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `${import.meta.env.VITE_REACT_APP_BASE_URL}/api/v1/job/getmyjobs`,
      { withCredentials: true }
    );
...
export const deleteJob = (id) => async (dispatch) => {
  ...
  try {
    const response = await axios.delete(
      `${import.meta.env.VITE_REACT_APP_BASE_URL}/api/v1/job/delete/${id}`,
      { withCredentials: true }
    );
